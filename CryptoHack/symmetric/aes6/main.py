from typing import cast
import numpy as np

N_ROUNDS = 10

key = b"\xc3,\\\xa6\xb5\x80^\x0c\xdb\x8d\xa5z*\xb6\xfe\\"
ciphertext = b"\xd1O\x14j\xa4+O\xb6\xa1\xc4\x08B)\x8f\x12\xdd"
s_box = np.array(
    [
        ,
    ]
)

inv_s_box = np.array(
    [
        ,
    ]
)


def bytes2matrix(text):
    """Converts a 16-byte array into a 4x4 matrix."""
    return np.array(
        [list(text[i : i + 4]) for i in range(0, len(text), 4)], dtype=np.uint8
    )


def matrix2bytes(matrix: np.ndarray):
    """Converts 4x4 matrix into 16 length byte string."""
    return bytes(matrix.flatten())


def expand_key(master_key):
    """
    Expands and returns a list of key matrices for the given master_key.
    """

    # Round constants https://en.wikipedia.org/wiki/AES_key_schedule#Round_constants
    r_con = np.array(
        [
            0x00,
            0x01,
            0x02,
            0x04,
            0x08,
            0x10,
            0x20,
            0x40,
            0x80,
            0x1B,
            0x36,
            0x6C,
            0xD8,
            0xAB,
            0x4D,
            0x9A,
            0x2F,
            0x5E,
            0xBC,
            0x63,
            0xC6,
            0x97,
            0x35,
            0x6A,
            0xD4,
            0xB3,
            0x7D,
            0xFA,
            0xEF,
            0xC5,
            0x91,
            0x39,
        ]
    )

    # Initialize round keys with raw key material.
    key_columns = bytes2matrix(master_key)
    iteration_size = len(master_key) // 4

    # Each iteration has exactly as many columns as the key material.
    i = 1
    while len(key_columns) < (N_ROUNDS + 1) * 4:
        # Copy previous word.
        word = key_columns[-1]

        # Perform schedule_core once every "row".
        if len(key_columns) % iteration_size == 0:
            # Circular shift.
            word = np.roll(word, -1)
            # Map to S-BOX.
            word = s_box[word]
            # XOR with first byte of R-CON, since the others bytes of R-CON are 0.
            word[0] ^= r_con[i]
            i += 1
        elif len(master_key) == 32 and len(key_columns) % iteration_size == 4:
            # Run word through S-box in the fourth iteration when using a
            # 256-bit key.
            word = s_box[word]

        # XOR with equivalent word from previous iteration.
        word = word ^ key_columns[-iteration_size]
        key_columns = np.append(key_columns, np.array([word]), axis=0)

    key_columns = key_columns.astype(np.uint8)
    # Group key words in 4x4 byte matrices.
    return [key_columns[4 * i : 4 * (i + 1)] for i in range(len(key_columns) // 4)]


def inv_shift_rows(matrix):
    # Works for any square matrix, not just 4x4
    _, num_cols = matrix.shape
    for i in range(num_cols):
        matrix[:, i] = np.concatenate((matrix[-i:, i], matrix[:-i, i]), axis=0)
    return matrix


# learned from http://cs.ucsb.edu/~koc/cs178/projects/JT/aes.c
xtime = lambda a: (((a << 1) ^ 0x1B) & 0xFF) if (a & 0x80) else (a << 1)


def mix_single_column(matrix_col):
    # see Sec 4.1.2 in The Design of Rijndael
    t = matrix_col[0] ^ matrix_col[1] ^ matrix_col[2] ^ matrix_col[3]
    u = matrix_col[0]
    matrix_col[0] ^= t ^ xtime(matrix_col[0] ^ matrix_col[1])
    matrix_col[1] ^= t ^ xtime(matrix_col[1] ^ matrix_col[2])
    matrix_col[2] ^= t ^ xtime(matrix_col[2] ^ matrix_col[3])
    matrix_col[3] ^= t ^ xtime(matrix_col[3] ^ u)
    return matrix_col


def mix_columns(matrix):
    for i in range(4):
        matrix[i] = mix_single_column(matrix[i])
    return matrix


def inv_mix_columns(matrix):
    # see Sec 4.1.3 in The Design of Rijndael
    for i in range(4):
        u = xtime(xtime(matrix[i][0] ^ matrix[i][2]))
        v = xtime(xtime(matrix[i][1] ^ matrix[i][3]))
        matrix[i][0] ^= u
        matrix[i][1] ^= v
        matrix[i][2] ^= u
        matrix[i][3] ^= v

    matrix = mix_columns(matrix)
    return matrix


def decrypt(key, ciphertext):
    round_keys = expand_key(
        key
    )  # Remember to start from the last round key and work backwards through them when decrypting

    # Convert ciphertext to state matrix
    matrix = bytes2matrix(ciphertext)

    # Initial add round key step
    matrix = np.bitwise_xor(
        matrix, round_keys[N_ROUNDS], dtype=np.uint8, casting="unsafe"
    )

    for i in range(N_ROUNDS - 1, -1, -1):
        matrix = inv_shift_rows(matrix)
        matrix = inv_s_box[matrix]
        matrix = np.bitwise_xor(matrix, round_keys[i], dtype=np.uint8, casting="unsafe")
        if i > 0:
            matrix = inv_mix_columns(matrix)

    # Convert state matrix to plaintext
    plaintext = matrix2bytes(matrix).decode("utf-8", errors="replace")
    return plaintext


print(decrypt(key, ciphertext))
